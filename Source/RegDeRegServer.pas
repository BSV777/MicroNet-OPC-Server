...
begin
 myCLSIDString:=GUIDToString(CLASS_OPCServer);
 aReg:=nil;
 try
  aReg:=TRegistry.Create;
  aReg.RootKey:=HKEY_CLASSES_ROOT;
  aReg.OpenKey(name,true);
  aReg.WriteString('','MicroNet Data Access Server Version 2.0');
 finally
  aReg.CloseKey;
  aReg.Free;
 end;
...
aReg:=nil;
 ...
 try
  ...
  aReg.WriteString('','MicroNet OPC Data Access 2.0');
 finally
  aReg.CloseKey;
  aReg.Free;
 end;
 ...
 hr:=CreateComponentCategory(CATID_OPCDAServer20,'MicroNet OPC Data Access');
 if hr <> 0 then
  ;
 hr:=RegisterCLSIDInCategory(CLASS_OPCServer,CATID_OPCDAServer20);
 if hr <> 0 then
  ;
end;
...
{       this is only needed for Data Access Version 1.0 according
 ...
 hr:=UnRegisterCLSIDInCategory(CLASS_OPCServer,CATID_OPCDAServer20);
 if hr <> 0 then
  ;

 hr:=UnCreateComponentCategory(CATID_OPCDAServer20,'MicroNet OPC Data Access');
 if hr <> 0 then
  ;
end;
