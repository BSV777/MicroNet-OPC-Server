...
begin
  if (toWhat = 0) and (itemIndex in [94..125]) then MicroNetOPCServer.AsyncCmd[itemIndex - 93] := $4B52;  //Послать устройству команду СБРОС
  if (toWhat = 0) and (itemIndex in [1..32]) then MicroNetOPCServer.Calibr[itemIndex] := $01;             //Начать калибровку
  if (toWhat <> 0) and (itemIndex in [187..217]) then MicroNetOPCServer.AsyncCmd[itemIndex - 186] := $4B54; //Команда СТАРТ
  if (toWhat = 0) and (itemIndex in [187..217]) then MicroNetOPCServer.AsyncCmd[itemIndex - 186] := $4B58;//Команда СТОП
end;
